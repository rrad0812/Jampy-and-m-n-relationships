...
function Events10() { // r.catalogs.book 

	function on_field_get_text(field) {
		
		if (field.field_name === "ref") {
			var result = [];
			var ref_copy = task.ref.copy();
			ref_copy.open({where: {id__in: field.value}})
	
			ref_copy.each(function(rec){
				result.push(' ' + rec.name.value);	
			})
			return result;	
		}
	}
